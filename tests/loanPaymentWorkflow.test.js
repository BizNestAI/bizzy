...
test("Books Review account dropdown exposes a manual loan split workflow safely", () => {
  ...
  const modal = read("src/components/Accounting/LoanPaymentSplitModal.jsx");
  ...
  assert.match(feed, /LoanPaymentSplitModal/);
  ...
  assert.match(mirror, /LoanPaymentSplitModal/);
  ...
  assert.match(modal, /export default function LoanPaymentSplitModal/);
  assert.match(modal, /ReactDOM\.createPortal/);
  assert.match(modal, /document\.body/);
  assert.match(modal, /role="dialog"/);
  assert.match(modal, /aria-modal="true"/);
  assert.match(modal, /w-\[min\(1180px,calc\(100vw-64px\)\)\]/);
  assert.match(modal, /max-h-\[min\(820px,calc\(100vh-64px\)\)\]/);
  assert.match(modal, /items-center justify-center/);
  assert.match(modal, /grid-cols-\[minmax\(300px,0\.4fr\)_minmax\(420px,0\.6fr\)\]/);
  assert.match(modal, /max-\[850px\]:grid-cols-1/);
  ...
  assert.match(modal, /Payment total/);
  assert.match(modal, /Allocated/);
  assert.match(modal, /Remaining/);
  assert.match(modal, /Loan details/);
  assert.match(modal, /Payment allocation/);
  assert.match(modal, /QuickBooks account/);
  assert.match(modal, /Add fee or another line/);
  assert.match(modal, /Remember this loan and description/);
