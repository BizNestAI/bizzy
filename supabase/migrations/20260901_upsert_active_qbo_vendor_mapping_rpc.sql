-- Active QBO Vendor mapping upsert.
-- The active mapping uniqueness is enforced by a partial unique index, so
-- application-level PostgREST upsert cannot infer the conflict target safely.
