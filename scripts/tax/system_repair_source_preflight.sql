-- Tax classification system_repair source preflight.
-- Read-only inspection script. Run before 20261003_tax_classification_system_repair_source.sql.
...
constraint_state as (
  select
    ...
    regexp_replace(pg_get_expr(c.conbin, c.conrelid), '\s+', ' ', 'g') =
      '(override_source = ANY (ARRAY[''user''::text, ''cpa''::text, ''admin''::text, ''system_correction''::text]))' as matches_expected_baseline
...
union all
select 'classification_baseline', jsonb_agg(row_to_json(t))
from (select * from classification_baseline) t
union all
