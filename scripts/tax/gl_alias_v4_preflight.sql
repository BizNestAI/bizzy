-- GL alias Tax classification v4 preflight.
-- Read-only inspection script. Run before the v4 reconciliation migration.
...
union all
select 'v2_deactivation_candidate_count', jsonb_agg(row_to_json(t)) from (
  select count(distinct r.rule_code)::integer as active_v2_rules_overlapped_by_canonical_v3
  from public.tax_deduction_rules r
