...
function shapeRule(rule, match_reason, match_score) {
  ...
  return {
    ...
    active: isActiveRule(rule),
    rule_kind: rule.rule_kind || null,
    created_at: rule.created_at || parseRuleNotes(rule.notes).created_at || null,
    updated_at: rule.updated_at || parseRuleNotes(rule.notes).updated_at || null,
    match_scope: rule.match_type === "merchant_entity_id" ? "provider_merchant" : "description_family",
